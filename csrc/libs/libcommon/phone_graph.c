               ...
               int multipron)
{
    ...
    for (i = 0; i < n_word; i++) {
        ...
        if (multipron) {
            ...
            if (chain_head != NULL) {
                /* Count variants by walking the chain. */
                lex_entry_t *e;
    ...
    for (i = 0; i < n_word; i++) {
        for (v = 0; v < plan[i].n_variants; v++) {
            ...
            for (p = 0; p < m; p++) {
                ...
                /* Intra-variant linear forward edges. */
                if (p < m - 1) {
                    set_next_one(g, slot, slot + 1);
                }
                /* Intra-variant linear backward edges. */
                if (p > 0) {
     ...
     * "last(i) -> first(i+1)" edge. */
    {
        ...
        for (i = 0; i < n_word; i++) {
...

            {
                ...
                if (nx > buf_cap) { buf_cap = nx; buf = ckd_realloc(buf, buf_cap * sizeof(uint32)); }
                for (k = 0; k < nx; k++) {
                    buf[k] = plan[next_active].first_slot[k];
                }
                for (v = 0; v < cur; v++) {
                    set_next_many(g, plan[i].last_slot[v], buf, nx);
                }

                if (cur > buf_cap) { buf_cap = cur; buf = ckd_realloc(buf, buf_cap * sizeof(uint32)); }
                for (v = 0; v < cur; v++) {
                    buf[v] = plan[i].last_slot[v];
                }
                for (k = 0; k < nx; k++) {
                    set_prior_many(g, plan[next_active].first_slot[k], buf, cur);
                }
            }
        }
        if (buf) ckd_free(buf);
