 ...
 *   4. Assembly: build the state_t array; set_next_state /
 *      set_prior_state are reused from state_seq.c (they're declared
 *      static there, so we provide tiny local equivalents here).
 *
...
/* For every state, compute the in-degree and out-degree, and the
           ...
           uint32 *total_prior)
{
    ...
    for (i = 0; i < graph->n; i++) {
        ...
        /* Intra-phone forward arcs: each emit state j has one arc per
         * nonzero tmat[j][k]. */
        for (j = 0; j < n_ms - 1; j++) {
            for (k = 0; k < n_ms; k++) {
                if (mt[j][k] > 0.0f) {
        ...
        /* Intra-phone backward arcs: each state j has one prior arc
         * per nonzero tmat[k][j] for k in 0..n_ms-2. */
        for (j = 0; j < n_ms; j++) {
            for (k = 0; k < n_ms - 1; k++) {
                if (mt[k][j] > 0.0f) {
                    ++tp;
...
/* Fill the packed next_state/prior_state arrays in the order required
 * by the assembly pass: per phone slot, in state-index order. */
static void
           ...
           uint32 total_prior)
{
    ...
    for (i = 0; i < graph->n; i++) {
        ...
        /* Cross-phone incoming arc(s) to first state. Match the
         * existing single-pron format: write the predecessor's
         * exit-state index and tprob.
         *
         * Each predecessor's outgoing weight to this slot is
        ...
        /* Intra-phone backward arcs to each state (sub-call to
         * the cross-phone prior writes above, which already
         * filled the slot for state l). */
        for (j = 0; j < n_ms; j++) {
            for (k = 0; k < n_ms - 1; k++) {
                if (mt[k][j] > 0.0f) {
    ...
    (void) total_next;   /* silence unused-param warnings in NDEBUG */
    (void) total_prior;
}
                     ...
                     model_def_t *mdef)
{
    ...
    /* Pass 3: write arcs into the packed arrays. */
    ...
    /* Pass 4: assemble state_t array, mirroring state_seq.c's
